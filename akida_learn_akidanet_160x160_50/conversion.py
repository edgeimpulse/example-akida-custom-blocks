...
def convert_akida_to_tf_lite(model_float, model_quantized, dir_path,
    ...
    concrete_func_float = get_concrete_function(model_float, model_input_shape)
    concrete_func_quantized = get_concrete_function(model_quantized, model_input_shape)

    print('Converting float32 model...')
    tflite_model = convert_float32(concrete_func_float, model_float, dir_path, model_filenames_float)
    print('Converting float32 model OK')
    print('')

    print('Converting quantized model...')
    # no need to use tflite convert, already quantized beforehand and this function supports 8bit input
    tflite_quant_model = convert_float32(concrete_func_quantized, model_quantized, dir_path, model_filenames_quantised_int8)
    print('Converting quantized model OK')
    print('')

    return model_quantized, tflite_model, tflite_quant_model
